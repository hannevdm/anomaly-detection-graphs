...
print(f"GraphSAGE DGI + LOF Accuracy: {accuracy}")
print(f"GraphSAGE DGI + LOF Precision: {precision}")
print(f"GraphSAGE DGI + LOF Recall: {recall}")
print(f"GraphSAGE DGI + LOF F1 Score: {f1}")
print(f"GraphSAGE DGI + LOF PR-AUC: {pr_auc}")

timer(False, "LOF + GraphSAGE DGI")
...
plot_scores(lof_scores_known, y_labelled, title='LOF (GraphSAGE DGI)')

# Plot Confusion Matrix
plot_confusion_matrix(x_known.numpy(), y_labelled, y_pred_known, title="GraphSAGE DGI + LOF Confusion Matrix")
